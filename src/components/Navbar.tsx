          ...
          {navActions.map((item) => (
            <li key={item.label}>
              <button
                ...
                className="text-[#f7f7f7] hover:text-[#FBBE17] text-sm font-medium transition-colors duration-200 relative group"
              >
                {item.label}
                <span
                  className="absolute -bottom-1 left-0 w-0 h-0.5 group-hover:w-full transition-all duration-300"
                  style={{ backgroundColor: '#FBBE17' }}
                />
              </button>
            </li>
          ))}
        ...
        <ul className="px-6 pb-4 flex flex-col gap-1">
          {navActions.map((item) => (
            <li key={item.label}>
              <button
                onClick={() => handleAction(item.action)}
                className="block w-full text-left py-3 text-white hover:text-[#FBBE17] text-lg font-bold border-b border-white/10 transition-colors duration-200"
              >
                {item.label}
              </button>
            </li>
